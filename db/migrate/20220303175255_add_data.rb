class AddData < ActiveRecord::Migration[7.0]
  def change
    ...
    for i in 1..100
      body = []
      title = []
      # Generate a bunch of words for the body
      word_count = rand 20..80
      for w in 0 .. word_count
        body.push(words[rand 0..499])
      end
      # Generate some words for the title
      word_count = rand 2..12
      for w in 0 .. word_count
        title.push(words[rand 0..499])
      end
      # Create the article / blog post
      a = Article.new(body: body.join(" "), title: title.join(" "))
      a.save()

      # Create some comments on the blog post
      comment_count = rand 3..50
      for c in 0 .. comment_count
        body = []
        # Generate words for the comment
        word_count = rand 1..30
        for w in 0 .. word_count 
          body.push(words[rand 0..499])
        end
        # Save the comment
        ...
        if reactions < 10
          ...
          # Now it is 0-10 instead of 0-20
          reactions = reactions - 10
        end
        # Extra spicy comments get lots of reactions.
        case reactions
        when 7
          reactions = rand 100..200
        when 8
          reactions = rand 200..400
        when 9
          reactions = rand 400..800
        when 10
          reactions = rand 1000..4000
        end
        # Add reactions, each one gets its own record
        # Every record represents a unique user who has reacted.
        # In a theoretical future they could remove their reaction.
        if reactions > 0
          for r in 1 .. reactions
            # Randomly choose thumbs up, thumbs down, or heart
            kind = rand 1..3
